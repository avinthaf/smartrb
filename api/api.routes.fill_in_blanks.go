...
func handleCreateFillInBlankDeck(c *gin.Context) {
	// Check authorization and get user ID
	userId, ok := checkAuthorization(c)
	if !ok {
		// checkAuthorization already sent the error response
	...
	// Parse request body
	var deckReq fill_in_blanks.CreateFillInBlankDeckRequest

	if err := c.ShouldBindJSON(&deckReq); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"error":   "Invalid fill in blank deck request",
			"details": err.Error(),
		})
		return
	}

	fillInBlankDeck, err := fill_in_blanks.CreateFillInBlankDeck(db.Default(), deckReq.Title, deckReq.Description, user.Id)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error":   "Failed to create fill in blank deck",
			"details": err.Error(),
		})
		return
	}
	...
	err = categories.CreateProductCategories(db.Default(), fillInBlankDeck.Id, deckReq.CategoryIds)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
	...
	// Create fill in blanks if provided
	if len(deckReq.FillInBlanks) > 0 {
		createFillInBlanksReq := fill_in_blanks.CreateFillInBlanksRequest{
			DeckId:       fillInBlankDeck.Id,
			FillInBlanks: deckReq.FillInBlanks,
		}
		
		err = fill_in_blanks.CreateFillInBlanks(db.Default(), createFillInBlanksReq)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error":   "Failed to create fill in blanks",
				"details": err.Error(),
			})
			return
		}
	...
	c.JSON(http.StatusCreated, fillInBlankDeck)
